...
const prompts: Prompt[] = [
    ...
    },
    {
        ...
        prompt: "Al calificarse este codigo, considere que se va usar la libreria LiquidCrystal y que los puntos a evaludar son: 1. Configuracion correcta de los pines del sensor HC SR04 de ultrasonido debe conectar el TRIG en el pin 10 y el ECHO en el pin 9 del arduino; 2. Debe usar correctamente el sensor; 3. Debe convertir la medicion del sensor en metros. ; 4. Debe imprimir la medicion en pantalla. No importa tanto el formato ni que otra cosa imprima siempre que imprima la distancia en metros. Este es un ejemplo de lo que se espera: #include <Wire.h> #include <LiquidCrystal_I2C.h> LiquidCrystal_I2C lcd(0x27, 16, 2); #define TRIG_PIN 10 #define ECHO_PIN 9 void setup() { Serial.begin(9600); lcd.begin(); lcd.backlight(); pinMode(TRIG_PIN, OUTPUT); pinMode(ECHO_PIN, INPUT); lcd.setCursor(0, 0); lcd.print(\"Distancia:\"); } void loop() { digitalWrite(TRIG_PIN, LOW); delayMicroseconds(2); digitalWrite(TRIG_PIN, HIGH); delayMicroseconds(10); digitalWrite(TRIG_PIN, LOW); long duration = pulseIn(ECHO_PIN, HIGH); float distance = (duration * 0.000343) / 2; lcd.setCursor(0, 1); lcd.print(distance); lcd.print(\" m    \"); Serial.print(\"Distancia: \"); Serial.print(distance); Serial.println(\" m\"); delay(500); }"
    }
]
